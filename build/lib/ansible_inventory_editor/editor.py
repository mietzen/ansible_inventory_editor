...
class AnsibleInventoryEditorError(Exception):
    pass
class AnsibleInventoryEditor:
    ...
    def set_host(self, hostname, ip_address, ansible_python_interpreter, no_backup, backups_to_keep):
        if self.check_if_hostname_is_taken(hostname) and self.check_if_ip_is_taken(ip_address):
            logging.info(hostname + ': ' + str(ip_address) + ' unchanged')
        else:
            if self.check_if_hostname_is_taken(hostname):
                logging.warning('The Hostname: ' + hostname + ' already exists')

            try:
                if self.check_if_ip_is_taken(ip_address):
                    raise AnsibleInventoryEditorError('AnsibleInventoryEditor', 'IP: ' + ip_address + ' is already take by: ' + self.get_hostname_form_ip(ip_address))
