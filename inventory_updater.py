...
class InventoryAdhocUpdater:
    ...
    def _write_ansible_hosts_file(self, no_backup, backups_to_keep):
        if not no_backup:
            ...
            timeStamp =  datetime.datetime.fromtimestamp(modifiedTime).strftime("%Y-%m-%d_%H-%M-%S")
            os.rename(self._ansible_hosts_file,  self._ansible_hosts_file + '_' + timeStamp)
            
        with open( self._ansible_hosts_file, "w") as stream:
            try:
                yaml.safe_dump(self._hosts, stream, default_flow_style=False)
            except Exception:
                os.rename(self._ansible_hosts_file + '_' + timeStamp, self._ansible_hosts_file)
                raise
        
        if not no_backup:
            _, _, files = next(os.walk(os.path.dirname(self._ansible_hosts_file)))
            if len(files) > backups_to_keep:
                files.sort()
                os.remove(os.path.join(os.path.dirname(self._ansible_hosts_file), files[0]))
    ...
    def set_host(self, container_name, container_ip, ansible_python_interpreter, no_backup, backups_to_keep):
        ...
        for host_name, host_data in self._hosts['all']['children']['lxc']['hosts'].items():
            if container_ip == host_data['ansible_host']:
                error('IP:' + host_data['ansible_host'] + 'is already take by: ' + host_name)
    ...
    def delete_host(self, container_name, no_backup, backups_to_keep):
        self._hosts['all']['children'][self._group]['hosts'].pop(container_name)
        self._write_ansible_hosts_file(no_backup, backups_to_keep)
...
# Parser
parser = argparse.ArgumentParser(prog='inventory_updater', description='This script will add or delete entries in your ansible inventory')
...
subparsers = parser.add_subparsers(title='subcommands', dest='command', help='For additional help type: inventory_updater COMMAND --help')

parser_print_hosts = subparsers.add_parser('print_hosts', help='Print all host and IP\'s of the group')

parser_print_ansible_hosts_file = subparsers.add_parser('print_hosts_file', help='Print the ansible hosts file')
parser_print_ansible_hosts_file.add_argument('--json', action='store_true', help='Print in json format')

parser_set_host = subparsers.add_parser('set_host', help='Add new host to the ansible hosts file')
parser_set_host.add_argument('container_name', metavar='NAME', type=str, help='Name of the container')
parser_set_host.add_argument('container_ip', metavar='IP', type=str, help='IP of the container')
parser_set_host.add_argument('--path', metavar='PATH', default='/usr/bin/python3', type=str, help='Ansible python interpreter path on host')
parser_set_host.add_argument('--keep', metavar='N', default=10, type=int, help='Backups to keep of the ansible hosts file')
parser_set_host.add_argument('--no_backup', action='store_true', help='DON\'T (!!!) Backup ansible hosts file')

parser_delete_host = subparsers.add_parser('delete_host', help='Delete a host from the ansible hosts file')
parser_delete_host.add_argument('container_name', metavar='NAME', type=str, help='Name of the container')
parser_delete_host.add_argument('--keep', metavar='N', default=10, type=int, help='Backups to keep of the ansible hosts file')
parser_delete_host.add_argument('--no_backup', action='store_true', help='DON\'T (!!!) Backup ansible hosts file')
